...
import Exceptions.*;
import Model.Procedure_Memory.inf;
...
public class Procedure_Memory {
	...
	public Map<Action, Double> findSuitableRules(Chunk chunk) throws ConditionRuleException, NoSuchRuleException{
		...
		Map<String, Double> strategyProbabilty = strategyProbabilty(chunk);
		for (cond[] conditions : rules.keySet()) {
			...
			for(cond individualCond : conditions) {
				...
				if(left.equals("operand"))
					...
					left=String.valueOf(strategyProbabilty.get("production"));
					//left=String.valueOf(estimationTimeProduction(chunk));
				else if(left.equals("answer"))
					left=String.valueOf(strategyProbabilty.get("answer"));
				...
				if(right instanceof String) {
					double rightValue = (right.equals("production")?
							strategyProbabilty.get("production") :
								strategyProbabilty.get("answer"));
					match=
	...
	private double estimationTimeAnswer(Chunk chunk) {
		...
		if(Answer_Memory.getInstance().getMemory().get(chunk.letter + "+" + chunk.number)!=null && Answer_Memory.getInstance().getMemory().get(chunk.letter + "+" + chunk.number).size()!=0) {
			...
			for(Answer_Memory.Answer answer : ans) {
				if(answer.getName().equals(String.valueOf((char)(Integer.valueOf(chunk.letter.charAt(0))+Integer.valueOf(String.valueOf(chunk.number)))))) {
					practice = answer.getPractice();
					flag=true;
				}
			}
			if(flag) {
				...
				if(times.get(chunk.letter+"+"+chunk.number) == null || times.get(chunk.letter+"+"+chunk.number)==0)
					return -1;
				return times.get(chunk.letter+"+"+chunk.number) + Parameters.switching_cost*(Parameters.other_strat-1)*(-1);
			}
		}
		else {
			//Sinon j'estime que le temps que ca me prendrait c'est le temps moyen que je prends a calculer le probleme
	...
	private double estimationTimeProduction(Chunk chunk) {
		...
		//Je récupère la liste des 5 dernières fois où j'ai compté le problème
		List<double[]> lastTime = Answer_Memory.getInstance().getLastTimeProduction(Integer.valueOf(chunk.number));
		if(chunk.problem.model.session== 10 && chunk.problem.strategyChosen.equals("null")) {
		}
		//Je ne récupère que les temps effectivement calculés (j'ignore les infinis)
		List<Double> values = new ArrayList<>();
	    for (double[] pair : lastTime) {
	        if (pair[0] == Double.POSITIVE_INFINITY) {
	            break; // on arrête dès qu'on voit l'infini
	        }
	        values.add(pair[0]);
	    }
	    ...
	    double medianLastTime=0;
	    // Comme la liste est déjà triée, pas besoin de trier à nouveau
	    if (n % 2 == 1) {
	    	medianLastTime= values.get(n / 2);
	    } else {
	    	medianLastTime = (values.get(n / 2 - 1) + values.get(n / 2)) / 2.0;
	    }
	...
	public Map<String, Double> strategyProbabilty(Chunk chunk){
		...
		Map<String, Double> decisionMap = new HashMap<String, Double>();
		
		if(estimationTimeAnswer == -1 || estimationTimeProduction == -1) {
			decisionMap.put("production", -1.0);
			decisionMap.put("answer", -1.0);
			return decisionMap;
		}
		double[] probabilities = softmax(estimationTimeAnswer,estimationTimeProduction);
		double retrievingProbability = probabilities[0];
		double countingProbability =  probabilities[1];
		double rdm = Math.random();
		double retrievingFinalDecision;
		double countingFinalDecision;
		
		if(rdm<=retrievingProbability) {
			retrievingFinalDecision=0;
			countingFinalDecision=1;
		}
		else {
			retrievingFinalDecision=1;
			countingFinalDecision=0;
		}		
		if(chunk.problem.strategyChosen.equals("null")) {
		double decisionTime = decisionTime(countingProbability, retrievingFinalDecision);
		chunk.problem.decisionTime=decisionTime;
		}
		decisionMap.put("production", countingFinalDecision);
		decisionMap.put("answer", retrievingFinalDecision);
		return decisionMap;
	}
	...
	public double[] softmax(double retrievingTime, double countingTime) {
		...
		double probaCounting= Math.exp(-rationality*countingTime) / ( Math.exp(-rationality*retrievingTime) + Math.exp(-rationality*countingTime));
		return(new double[] {probaRetrieving, probaCounting});
	}
	...
	public double decisionTime(double calculateProba, double retrieveProba) {
		return 0;
	}
