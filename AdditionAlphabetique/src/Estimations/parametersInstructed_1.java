...
public class parametersInstructed_1{
	...
	public static void main(String[] args) {
		...
		int totalParamComb = 10 * 20 * 20 * 10; 
		int totalLines = totalParamComb * 4;   


		int totalIterations = totalLines;

		double decisionDeterminism=0;
		if (args.length > 0) {
            decisionDeterminism = Double.parseDouble(args[0]);
            // Tu peux maintenant utiliser `valeur`
        } else {
            System.out.println("Aucun paramètre fourni.");
            return;
        }
		...
		String csvFile = "estimationInstructed" + String.valueOf(decisionDeterminism).replace(".", "_") + ".csv";
		...
		try {
			...
			try (FileWriter writer = new FileWriter(file)) {
				writer.append("decisionDeterminism, errorDiscount, weightIncrease, increasePractice, increaseStrength, Addend, accuracy, rt, profil\n");
				...
				//for(double decisionDeterminism=1; decisionDeterminism<=1.5; decisionDeterminism=Math.round((decisionDeterminism+0.1)*10.0)/10.0) {

					for(double errorDiscount=0.01;errorDiscount<=0.1;errorDiscount=Math.round((errorDiscount+0.01)*100.0)/100.0){
						for(double wi=0.5;wi<=10;wi=Math.round((wi+0.5)*10.0)/10.0) {
							for(double incrStrength=0.5;incrStrength<=10;incrStrength=Math.round((incrStrength+0.5)*10.0)/10.0) {
								for(int incrPrac=1;incrPrac<=10;incrPrac++) {
									for(int i=0;i<5;i++) {
										model.cleanAnswerMemory();
										model.cleanProcedureMemory();
										model=new Model(decisionDeterminism, errorDiscount, wi, 0, incrPrac, 0, incrStrength, "breaker");
										for(int a=0;a<288;a++) {
											try {
									...
									for(int ad=2;ad<6;ad++) {
										...
										writer.append(String.format(Locale.US,"%.2f,%.2f,%.2f,%d,%2f,%d,%.5f,%.2f,%s\n",
												decisionDeterminism, errorDiscount, wi,incrPrac,incrStrength, ad, e, ti, "b"));
										iterationCount++;
									}
									...
									for(int i=0;i<5;i++) {
										model.cleanAnswerMemory();
										model.cleanProcedureMemory();
										model=new Model(decisionDeterminism, errorDiscount, wi, 0, incrPrac, 0, incrStrength, "nonbreaker");
										for(int a=0;a<288;a++) {
											try {
												char randomLetter = letters[new Random().nextInt(letters.length)];
												int randomNumber = (int) ((Math.random() * (5 - 2 + 1)) + 2);
									...
									for(int ad=2;ad<6;ad++) {
										...
										double ti=(double)(timeMap.get(ad+"")+0.0)/nbPassages[ad-2];
										writer.append(String.format(Locale.US,"%.2f,%.2f,%.2f,%d,%2f,%d,%.5f,%.2f, %s\n",
												decisionDeterminism, errorDiscount, wi,incrPrac,incrStrength, ad, e, ti, "nb"));
										iterationCount++;
									}
									timeMap=new HashMap<String, Double>();
									errorMap=new HashMap<String,Integer>();
