...
public class parametersEstimation_3 {
	...
	public static void main(String[] args) {
		...
		String csvFile = "3_CSCproblemsParameters.csv";
		...
		try {
			...
			try (FileWriter writer = new FileWriter(file)) {
				...
				//1 et 1.1
				for(double decisionDeterminism=1; decisionDeterminism<1.2; decisionDeterminism=decisionDeterminism+0.1) {
					for(double errorDiscount=0;errorDiscount<1;errorDiscount=errorDiscount+0.1){
						if(decisionDeterminism==1){
									...
									for(double initStrength=0;initStrength<2;initStrength=initStrength+0.5) {
										for(double incrStrength=0.5;incrStrength<2.5;incrStrength=incrStrength+0.5) {
											...
											if(!oneSimulationDone) {
												...
												double timeTotal = timeOneSim/1000*2*1*4*4*5*4*4;
