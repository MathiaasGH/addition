...
public class parametersEstimation_2 {
	...
	public static void main(String[] args) {
		...
		String csvFile = "2_CSCproblemsParameters.csv";
		...
		try {
			...
			try (FileWriter writer = new FileWriter(file)) {
				...
				//0.8 et 0.9
				for(double decisionDeterminism=0.8; decisionDeterminism<1; decisionDeterminism=decisionDeterminism+0.1) {
					for(double errorDiscount=0;errorDiscount<1;errorDiscount=errorDiscount+0.1){
						if(decisionDeterminism==0.8){
								...
								for(int incrPract=1;incrPract<6;incrPract=incrPract+1) {
									for(double initStrength=0;initStrength<2;initStrength=initStrength+0.5) {
										for(double incrStrength=0.5;incrStrength<2.5;incrStrength=incrStrength+0.5) {
											...
											//Je cherche la moyenne des temps par addend et par session
