...
public class parametersEstimation_1 {
	...
	public static void main(String[] args) {
		...
		String csvFile = "1_CSCproblemsParameters.csv";
		...
		try {
			...
			try (FileWriter writer = new FileWriter(file)) {
				...
				//0.6 et 0.7
				for(double decisionDeterminism=0.6; decisionDeterminism<0.8; decisionDeterminism=decisionDeterminism+0.1) {
					for(double errorDiscount=0;errorDiscount<1;errorDiscount=errorDiscount+0.1){
						if(decisionDeterminism==0.6){
										...
										for(double incrStrength=0.5;incrStrength<2.5;incrStrength=incrStrength+0.5) {
											for(int i=0;i<10;i++) {
												...
												for(int session=1; session<11; session++) {
													for(int a=0;a<288;a++) {
														...
														Problem problem = new Problem((char)(randomLetter) + "+" + randomNumber , model, "CSC","breaker");
											...
											for(Integer sessions : time.keySet()) {
												...
												for(Integer addends : timeBySession.keySet()) {
													...
													writer.append(String.format(Locale.US,"%.2f,%.2f,%.2f,%d,%d,%.2f,%.2f,%d,%.2f,%d\n",
															decisionDeterminism, errorDiscount, wi, initPrac, incrPract,
															initStrength, incrStrength, addends, meanTime, sessions));
												}
											}
