...
public class parametersEstimation_5 {
	...
	public static void main(String[] args) {
		...
		String csvFile = "5_CSCproblemsParameters.csv";
		...
		try {
			...
			try (FileWriter writer = new FileWriter(file)) {
				...
				//1.4 et 1.5
				for(double decisionDeterminism=1.4; decisionDeterminism<1.6; decisionDeterminism=decisionDeterminism+0.1) {
					for(double errorDiscount=0;errorDiscount<1;errorDiscount=errorDiscount+0.1){
						if(decisionDeterminism==1.4){
							System.out.println((errorDiscount*100)/2 + "%");
						}
						else {
							System.out.println((50 + (errorDiscount*100)/2) + "%");
						}
						for(double wi=0.5;wi<2;wi=wi+0.5) {
