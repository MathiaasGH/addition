...
public class Answer_Memory {
	...
	List<List<double[]>> lastTimeAddend;
	...
	private Answer_Memory() {
		...
		lastTimeAddend = new ArrayList<List<double[]>>();
		for(int nbAddend=0;nbAddend<4;nbAddend++) {
			List<double[]> emptyList = new ArrayList<double[]>();
			lastTimeAddend.add(emptyList);
			for(int nbTimeByAddend=0;nbTimeByAddend<5;nbTimeByAddend++) {
				double[] emptyCouple = new double[] {Double.POSITIVE_INFINITY, nbTimeByAddend+1};
				lastTimeAddend.get(nbAddend).add(emptyCouple);
			}
		}
	}
	...
	public String[] findAnswer(Chunk chunk) throws CantRetrieveAnswerInAnswerMemoryException{
		...
		//au temps pris pour résoudre le chunk, puis le retourne la réponse trouvée
			...
			//Je récupère l'indice de la réponse
			int idx = answer.charAt(0) - 'a';
			List<Answer> answerList = weightList[idx];
			double maxActivation = answerList.get(0).getWeight()*similarity(answerList.get(0).getName(), chunk.letter+"+"+chunk.number);
			String problemSolved=answerList.get(0).getName();
			for(Answer ans : answerList) {
				double ansActivation = ans.getWeight()*similarity(ans.getName(), chunk.letter+"+"+chunk.number);
				if(ansActivation >= maxActivation) {
					maxActivation = ansActivation;
					problemSolved=ans.getName();
				}
			}
			double practice=0;
			...
			probabilityMap = makeProbabilityMap(potentialAnswers, activationMap);
			...
			System.out.println("I try to resolve " + chunk.letter+"+"+chunk.number);
	...
	/**
	 * Crée une map associant une réponse avec la probabilité de la récupérer
	 * @param potentialAnswers les réponses potentielles
	 * @param activationMap la map d'activation des réponses
	 * @return une map associant une réponse avec la probabilité de la récupérer
	 */
	private HashMap<String, Double> makeProbabilityMap(List<String> potentialAnswers, Map<String,Double> activationMap) {
		HashMap<String, Double> probabilityMap = new HashMap<String, Double>();
		//Pour chaque réponse que le modèle pourrait trouver
		for(String potentialAnswer : potentialAnswers) {
			...
			//Si une probabilité dépasse un seuil à tel point que sa valeur est NaN
			if(Double.isNaN(probability(potentialAnswer,potentialAnswers,activationMap))) {
				//Je divise les activation de chaque réponse par 10...
				activationMap=divideBy10(activationMap);
				probabilityMap = new HashMap<String, Double>();
				//et je recommence
				return makeProbabilityMap(potentialAnswers, activationMap);
			}
		}
	...
	/**
	 * Divise toutes les valeurs d'une map par 10
	 * @param map la map contenant les valeurs à diviser 
	 * @return la même map avec les valeurs divisées par 10
	 */
	private Map<String, Double> divideBy10(Map<String, Double> map) {
	 ...
	 */
	protected void putMemory(String problemName, String answer, String strategy, double time) {		
		double weight = Parameters.weightIncrease;
		//Je récupère les réponses au problème
		List<Answer> answerFromMemory = memory.get(problemName);
		//Je cherche la réponse que j'ai choisi
		if(answerFromMemory != null) {
			...
			for(Answer ans : answerFromMemory) {
				if(ans.getName().equals(answer)) {
					...
					ans.practice(strategy);
					flag=true;
				}
			}
			if(!flag) {
				//Je modifie le poids de la réponse car je l'ai travaillée
		...
		//Si j'ai compté pour trouver la réponse, je sauvegarde en mémoire de travail le temps que j'ai pris pour la compter
		if(strategy.equals("production")) {
			int addend=Integer.valueOf(problemName.charAt(2)+"");
			List<double[]> timeListByAddend = lastTimeAddend.get(addend-2);

			for(int i=0;i<timeListByAddend.size();i++) {
				double[] currentTime = timeListByAddend.get(i);
				if(currentTime[1]==1) {
					timeListByAddend.remove(i);
					break;
				}
			}
			double[] newTime = new double[] {time,5};
			for(int i=0;i<timeListByAddend.size();i++) {
				double[] currentTime=timeListByAddend.get(i);
				currentTime[1]=currentTime[1]-1;
			}
			timeListByAddend.add(newTime);

			Collections.sort(timeListByAddend, new Comparator<double[]>() {
	...
	protected class Answer{
		...
		private Answer(String name, double weight) {
			this.name=name;
			this.weight=weight;
			assoStrength=Parameters.increaseStrength;
		}
		...
		private void practice(String strategy) {
			if(strategy.equals("production"))
				assoStrength= assoStrength+Parameters.counting_reinforcement;
			else if (strategy.equals("answer")) {
				assoStrength= assoStrength+Parameters.retrieving_reinforcement;
			}
		}
	...
	/**
	 * Récupère le temps moyen (appelé dernier temps) de résolution d'un problème d'addend fixé
	 * @param addend l'addend voulu
	 * @return le temps moyen de résolution d'un problème quelconque ayant l'addend demandé
	 */
	protected List<double[]> getLastTimeProduction(int addend) {
		if(addend>5 || addend<2) {
			List<double[]> fakeList = new ArrayList<double[]>();
			for(int i=1;i<6;i++)
				fakeList.add(new double[]{Double.POSITIVE_INFINITY, i});
			return fakeList;
		}
		return lastTimeAddend.get(addend-2);
	}
